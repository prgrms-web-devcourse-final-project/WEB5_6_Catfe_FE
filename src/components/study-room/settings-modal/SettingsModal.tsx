...
const MENUS: SidebarItem[] = [
  {
    key: 'general',
    label: '일반',
    iconAct: '/icon/study-room/settings.svg',
    iconNon: '/icon/study-room/settings-non.svg',
  },
  {
    key: 'security',
    label: '보안',
    iconAct: '/icon/study-room/lock.svg',
    iconNon: '/icon/study-room/lock-non.svg',
  },
  {
    key: 'roles',
    label: '멤버 권한',
    iconAct: '/icon/study-room/profile.svg',
    iconNon: '/icon/study-room/profile-non.svg',
  },
  {
    key: 'notice',
    label: '알림',
    iconAct: '/icon/study-room/bell.svg',
    iconNon: '/icon/study-room/bell-non.svg',
  },
];
...
export default function SettingsModal({ open, onClose, defaultTab = 'general', roomId }: Props) {
  ...
  const { data: me, isLoading: isMeLoading } = useUser();
  ...
  const isPending = isMeLoading || isLoading;
  ...
  return (
    ...
    >
      <BigModal.Body className="p-0 h-auto overflow-hidden">
        {isPending && <div className="p-6 text-sm text-text-secondary">방 정보를 불러오는 중…</div>}
