...
import { getEnterRooms } from "@/api/apiRooms";
...
export default function EnterRoomsList() {
  ...
  const fetchServerPage = useCallback(async (oneBasePage: number) => {
    ...
    try {
      ...
      const res = await getEnterRooms(zeroBase, UI_PAGE_SIZE);
      setRows(res.content);
  ...
  const fetchAllThenFilter = useCallback(async () => {
    ...
    setError(null);
    try {
      const first = await getEnterRooms(0, SERVER_PAGE_SIZE);
      let acc = first.content.slice();
      for (let p = 1; p < (first.totalPages || 1); p++) {
        const r = await getEnterRooms(p, SERVER_PAGE_SIZE);
  ...
  return (
    <>
      {noData ? (
        <div className="w-full py-20 text-center text-text-secondary">아직 입장 가능한 공개방이 없어요 😿</div>
      ) : noMatch ? (
